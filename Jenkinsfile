...
pipeline {
    ...
    stages {
        stage('git') {
            steps {
                git branch: 'dev', url: 'https://github.com/QuantumQuackDoctor/Gateway.git'
            }
        }
        stage('build') {
            steps {
        ...
        stage('docker') {
            steps{
                script {
                    sh 'cp -r /var/lib/jenkins/workspace/gateway-job/users-api/target .'
                    sh 'docker build . -t quangmtran36/qqd-gateway:$Docker_tag'
